document.addEventListener("DOMContentLoaded", function () {
  const customParcelsNodesModal = new bootstrap.Modal(
    document.getElementById("customParcelsNodesModal"),
    {
      backdrop: "static",
      keyboard: false,
    }
  ...
  document
    .querySelectorAll("#showCustomParcelsNodesModal")
  ...
  function loadParcelsNodesMap(projectId) {
    ...
    fetch(`/dashboard_super/get_project_details/${projectId}/`)
      ...
      .then((projectData) => {
        ...
        const map = (window.customMap = L.map(mapElement).setView(
          [latitude, longitude],
          15
        )); // Center on project coordinates
        ...
        document
          .getElementById("customParcelsNodesUpdateButton")
  ...
  function updateParcelsNodes(projectId, drawnItemsPolygon, drawnItemsMarker) {
    ...
    const coordinatesPolygon = layersPolygon.map((layer) =>
