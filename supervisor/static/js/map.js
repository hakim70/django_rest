document.addEventListener("DOMContentLoaded", function () {
  window.onload = function () {
    ...
    const modalTitle2 = document.getElementById(
      "customDisplayParcelsModalLabel"
    );
    ...
    if (projectSelect) {
      projectSelect.addEventListener("change", function () {
        ...
        const latitude = parseFloat(
          selectedOption.getAttribute("data-latitude")
        );
    ...
    function fetchParcellesForProject(projectId) {
      fetch(
        `/dashboard_super/get_parcelles_for_project/?project_id=${projectId}`
      )
        .then((response) => {
          ...
          if (data.parcelles.length > 0) {
            ...
            data.parcelles.forEach((parcelle) => {
              if (
                ...
                console.error(
                  `Invalid coordinates for parcelle ID: ${parcelle.id}`,
      ...
      nextButtonPolygon.addEventListener("click", function () {
        ...
        fetch(url, {
          ...
          body: `name=${encodeURIComponent(
            nameValue
          )}&coordinates=${JSON.stringify(coordinates)}&project=${projectId}`,
        })
          .then((response) => response.json())
          .then((data) => {
            if (data.message) {
              // Hide the first modal
              const projectMapModal = bootstrap.Modal.getInstance(
                document.getElementById("projectMapModal")
              );
              if (projectMapModal) {
              ...
              const displayParcelsModal = new bootstrap.Modal(
                document.getElementById("displayParcelsModal"),
                {
                  backdrop: "static",
                  keyboard: false,
                }
              ...
              if (lastParcelle) {
                document
                  .getElementById("id_parcelle")
                  .setAttribute("parcelle-id", lastParcelle.id);
              } else {
    ...
    function loadParcelsOnDisplayMap(projectId) {
      ...
      fetch(
        `/dashboard_super/get_parcelles_with_nodes_for_project/?project_id=${projectId}`
      )
        .then((response) => response.json())
        .then((data) => {
      ...
      function updatePositionFields(latlng) {
        ...
        if (positionField) {
          positionField.value = `POINT(${latlng.lng.toFixed(
            6
        ...
        console.log(
          `Updated Position: ${positionField.value}, Latitude: ${latitudeField.value}, Longitude: ${longitudeField.value}, Parcelle: ${parcelleField.value}`
      ...
      if (nextButtonMarker) {
        nextButtonMarker.addEventListener("click", function () {
          ...
          document.getElementById(
            "nodePosition"
          ).value = `POINT(${coordinates.lng.toFixed(
            6
          ...
          console.log(
            `Submitting Node: Name=${nameValue}, Reference=${nodeReferenceValue}, Sensors=${nodeSensorsValue}, Order=${nodeOrderValue}, Position=${
              document.getElementById("nodePosition").value
            }, Latitude=${
              document.getElementById("id_latitude").value
            }, Longitude=${
              document.getElementById("id_longitude").value
            }, Parcelle=${parcelleInput.value}`
          ...
          fetch("/dashboard_super/add_node/", {
            ...
            body: `name=${encodeURIComponent(
              ...
              nodeReferenceValue
            )}&sensors=${encodeURIComponent(
              nodeSensorsValue
            )}&node_range=${encodeURIComponent(
              nodeOrderValue
            )}&position=POINT(${coordinates.lng.toFixed(
              6
            )} ${coordinates.lat.toFixed(
              6
            )})&latitude=${coordinates.lat.toFixed(
              6
            )}&longitude=${coordinates.lng.toFixed(
              6
            )}&parcelle=${encodeURIComponent(
            ...
            .then((data) => {
              if (data.message) {
                ...
                const displayParcelsModalInstance = bootstrap.Modal.getInstance(
                  document.getElementById("displayParcelsModal")
                );
                ...
                const customDisplayParcelsModal = new bootstrap.Modal(
                  document.getElementById("customDisplayParcelsModal"),
                  {
                    backdrop: "static",
                    keyboard: false,
                  }
